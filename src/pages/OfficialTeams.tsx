...
export const OfficialTeams = ({ isAdmin }: OfficialTeamsProps) => {
  ...
  const loadTeams = () => queryClient.invalidateQueries({ queryKey: ["official_teams"] });

  const reorderTeams = async (newOrder: Team[]) => {
    queryClient.setQueryData(["official_teams"], newOrder);
    const updates = newOrder.map((team, index) =>
      supabase.from('official_teams').update({ sort_order: index }).eq('id', team.id)
    );
          ...
          <CardContent className="p-8">
            {isLoading ? (
              ...
              <div className="text-center py-12">
                ...
                <p className="text-sm text-muted-foreground">Official teams will be listed here once they're added.</p>
              </div>
            ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              <SortableList
