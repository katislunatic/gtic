...
Deno.serve(async (req) => {
  ...
  try {
    ...
    for (const { category, member: m } of assignments.values()) {
      ...
      // Prefer server-specific banner/decoration (set separately per-server on Discord)
      // over the member's global, cross-server profile. Only fall back to the global
      // user profile if they haven't customized anything for this server.
      if (m.banner) {
        banner = cdn(`guilds/${guildId}/users/${user.id}/banners`, m.banner)
      }
      const guildDecoration = m.avatar_decoration_data
      if (guildDecoration?.asset) {
        decoration = `https://cdn.discordapp.com/avatar-decoration-presets/${guildDecoration.asset}.png?size=160&passthrough=false`
      }

      // Only hit the global user profile if something's still missing —
      // most members won't have set a server-specific banner/decoration.
      if (!banner || !decoration) {
        const profile = await fetch(`${DISCORD_API}/users/${user.id}`, {
          headers: { Authorization: `Bot ${token}` },
        })
        if (profile.ok) {
          const u = await profile.json()
          if (!banner && u.banner) banner = cdn(`banners/${user.id}`, u.banner)
          if (!decoration && u.avatar_decoration_data?.asset) {
            decoration = `https://cdn.discordapp.com/avatar-decoration-presets/${u.avatar_decoration_data.asset}.png?size=160&passthrough=false`
          }
        }
      }

      // Note: Profile Effects and Profile Frames are NOT available through the bot
      // API — Discord only exposes those through a private, user-token-only endpoint
      // the official client uses. There's no supported way for a bot to read them,
      // so they can't be synced here no matter what.
